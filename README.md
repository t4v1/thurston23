...
* `CatalanLattice.lean` — the finite shadow of the transcendence of `G`: for each degree
  `d ≤ 10`, `G` is not a root of a nonzero integer polynomial of degree `≤ d` with
  coefficients bounded by an explicit `H(d)`, from a 46-decimal enclosure of `G` and
  LLL lattice certificates checked by `decide +kernel`. Imports `CatalanEisensteinRatio`.
  The certificates are regenerated by `tools/catalan_lattice/gen_catalan_lattice.py`.
...
`Thurston23Eisenstein`, which imports the last two, `CatalanEisensteinRatio`, and
`CatalanLattice`, which imports it; all six are declared as `lean_lib` targets in
`lakefile.toml`, and Mathlib is not rebuilt.
